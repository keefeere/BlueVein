...
BlueVein автоматически синхронизирует ключи сопряжения. Повторное подключение
зависит от Bluetooth-стека каждой ОС: правильные ключи сами по себе не исправят
ошибку подключения на хосте.

Если имя устройства известно хотя бы одной ОС, BlueVein хранит его в общей
записи. Windows читает кэш имён BTHPORT, а при его отсутствии — отображаемое
имя из PnP: для давних LE-сопряжений это часто единственный источник. Linux
читает `Name` из BlueZ, а при его отсутствии — пользовательский `Alias`, и
применяет общее имя как `Name` при импорте или обновлении сопряжения, сохраняя
локальный `Alias`. Найденное позже имя добавляется в существующую запись, даже
если ключи этого устройства импортировать нельзя. В журнале устройства
обозначаются адресом и именем. Имена необязательны: старые общие файлы и
устройства без известного имени остаются рабочими.
...
| 💾 **Доступ к EFI** | В Linux использует смонтированную EFI-файловую систему, если она доступна; иначе [fat32-raw](https://github.com/meowrch/fat32-raw) |
...
- **При удалении:** Если сопряжение исчезло после того, как BlueVein наблюдал его в этой ОС, в EFI появляется отметка об ожидающем удалении. Другая ОС удаляет сопряжение только при совпадении ключей, после чего запись EFI очищается. Отсутствие сопряжения при запуске не считается удалением. Неудачное удаление не останавливает синхронизацию остальных устройств: отметка сохраняется, ошибка сообщается после цикла, и попытка повторяется. Если Windows сообщает, что такого сопряжённого устройства у неё нет, удаляются только остатки ключей в BTHPORT. Подробности — в [описании протокола](docs/windows-le-sync.md#pairing-removal-protocol-candidate-2026-09-20).
...
| ❌ **Ключи совпадают, но Linux не подключается** | Проверь соединение LE/Classic и профиль HID отдельно. BlueVein синхронизирует ключи, но не сканирует и не подключает устройства. У BlueZ 5.87 есть [ошибка переподключения dual-mode устройств с приватным адресом](https://github.com/bluez/bluez/issues/2356), для которой нужен отдельный обход на хосте. |
