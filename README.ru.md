...
- **При удалении:** Если сопряжение исчезло после того, как BlueVein наблюдал его в этой ОС, в EFI появляется отметка об ожидающем удалении. Другая ОС удаляет сопряжение только при совпадении ключей, после чего запись EFI очищается. Отсутствие сопряжения при запуске не считается удалением; подробности — в [описании протокола](docs/windows-le-sync.md#pairing-removal-protocol-candidate-2026-09-20).
